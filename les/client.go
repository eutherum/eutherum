...
// Package les implements the Light Eutherum Subprotocol.
package les
...
import (
	...
	"github.com/eutherum/eutherum/les/vflux"
	vfc "github.com/eutherum/eutherum/les/vflux/client"
...
func New(stack *node.Node, config *ethconfig.Config) (*LightEutherum, error) {
...
func (s *LightEutherum) VfluxRequest(n *enode.Node, reqs vflux.Requests) vflux.Replies {
	if !s.udpEnabled {
		return nil
	}
...
func (s *LightEutherum) vfxVersion(n *enode.Node) uint {
	if n.Seq() == 0 {
		var err error
		if !s.udpEnabled {
			return 0
...
func (s *LightEutherum) prenegQuery(n *enode.Node) int {
	if s.vfxVersion(n) < 1 {
...
// APIs returns the collection of RPC services the eutherum package offers.
// NOTE, some of these services probably need to be moved to somewhere else.
func (s *LightEutherum) APIs() []rpc.API {
...
func (s *LightEutherum) ResetWithGenesisBlock(gb *types.Block) {
	s.blockchain.ResetWithGenesisBlock(gb)
}

func (s *LightEutherum) BlockChain() *light.LightChain      { return s.blockchain }
func (s *LightEutherum) TxPool() *light.TxPool              { return s.txPool }
func (s *LightEutherum) Engine() consensus.Engine           { return s.engine }
func (s *LightEutherum) LesVersion() int                    { return int(ClientProtocolVersions[0]) }
func (s *LightEutherum) Downloader() *downloader.Downloader { return s.handler.downloader }
func (s *LightEutherum) EventMux() *event.TypeMux           { return s.eventMux }

// Protocols returns all the currently configured network protocols to start.
func (s *LightEutherum) Protocols() []p2p.Protocol {
...
// light eutherum protocol implementation.
func (s *LightEutherum) Start() error {
...
// Eutherum protocol.
func (s *LightEutherum) Stop() error {
	close(s.closeCh)
	s.serverPool.Stop()
	s.peers.close()
	s.reqDist.close()
	...
	s.lesDb.Close()
	s.wg.Wait()
	log.Info("Light eutherum stopped")
	return nil
}
