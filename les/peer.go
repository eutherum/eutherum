...
import (
	...
	"github.com/eutherum/eutherum/les/utils"
...
type PeerInfo struct {
	Version    int      `json:"version"`    // Eutherum protocol version negotiated
...
func (p *peerCommons) handshake(td *big.Int, head common.Hash, headNum uint64, genesis common.Hash, forkID forkid.ID, forkFilter forkid.Filter, sendCallback func(*keyValueList), recvCallback func(keyValueMap) error) error {
	...
	// as well. Check http://eips.eutherum.org/EIPS/eip-2124 for more
	// spec detail.
	if p.version >= lpv4 {
	...
	return p.handshake(td, head, headNum, genesis, forkID, forkFilter, func(lists *keyValueList) {
		...
		if !server.config.UltraLightOnlyAnnounce {
			...
			*lists = (*lists).add("serveStateSince", uint64(0))
			...
			// If local eutherum node is running in archive mode, advertise ourselves we have
...
// participating in the Light Eutherum sub-protocol.
...
// for us(server) but for other protocols(e.g. UTH) upon the devp2p they
// may be useful.
