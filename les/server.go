...
type LesServer struct {
	...
	archiveMode bool // Flag whether the eutherum node runs in archive mode.
...
func NewLesServer(node *node.Node, e ethBackend, config *ethconfig.Config) (*LesServer, error) {
	...
	srv.euracle = srv.setupEuracle(node, e.BlockChain().Genesis().Hash(), config)
	...
	srv.vfluxServer.Register(srv.clientPool, "les", "Eutherum light client service")
