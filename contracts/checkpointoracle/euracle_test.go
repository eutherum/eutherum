...
import (
	...
	"github.com/eutherum/eutherum/accounts/abi/bind/backends"
...
func validateOperation(t *testing.T, c *contract.CheckpointEuracle, backend *backends.SimulatedBackend, operation func(),
	assert func(<-chan *contract.CheckpointEuracleNewCheckpointVote) error, opName string) {
	// Watch all events and deliver them to assert function
	var (
		sink   = make(chan *contract.CheckpointEuracleNewCheckpointVote)
...
func TestCheckpointRegister(t *testing.T) {
	...
	contractAddr, _, c, err := contract.DeployCheckpointEuracle(transactOpts, contractBackend, []common.Address{accounts[0].addr, accounts[1].addr, accounts[2].addr}, sectionSize, processConfirms, big.NewInt(2))
	if err != nil {
	...
	}, func(events <-chan *contract.CheckpointEuracleNewCheckpointVote) error {
	...
	validateOperation(t, c, contractBackend, func() {
		...
		if valid, recv := validateEvents(2, events); !valid {
			...
			for i := 0; i < len(recv); i++ {
				event := recv[i].Interface().(*contract.CheckpointEuracleNewCheckpointVote)
