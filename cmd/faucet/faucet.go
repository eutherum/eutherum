...
// faucet is an Euther faucet backed by a light client.
package main
...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Eutherum protocol")
	statsFlag   = flag.String("ethstats", "", "Ethstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of euthers to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Eutherum and the faucet")
...
type request struct {
	...
	Account common.Address     `json:"account"` // Eutherum address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Eutherum light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Eutherum protocol stack
	client *ethclient.Client   // Client connection to the Eutherum chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*enode.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Eutherum light client protocol
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to register the Eutherum service: %w", err)
	}
...
// close terminates the Eutherum connection and tears down the faucet.
...
// apiHandler handles requests for Euther grants and transaction statuses.
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		// Retrieve the Eutherum address to fund, the requesting user and a profile picture
		var (
			id       string
			username string
			avatar   string
		...
		default:
			//lint:ignore ST1005 This error is to be displayed in the browser
			err = errors.New("Something funky happened, please open an issue at https://github.com/eutherum/eutherum/issues")
		}
		if err != nil {
			if err = sendError(wsconn, err); err != nil {
...
// the uniqueness identifier (user id/username), username, avatar URL and Eutherum address to fund on success.
func authTwitter(url string, tokenV1, tokenV2 string) (string, string, string, common.Address, error) {
	...
	// and scrape it for the Eutherum address and profile URL. We need to load
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", "", common.Address{}, errors.New("No Eutherum address found to fund")
	}
	var avatar string
...
// API, returning the user id, username, avatar URL and Eutherum address to fund on
// success.
...
// returning the username, avatar URL and Eutherum address to fund on success.
func authFacebook(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Eutherum address and profile URL.
	//
...
// authNoAuth tries to interpret a faucet request as a plain Eutherum address,
