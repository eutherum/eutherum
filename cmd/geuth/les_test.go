...
func (g *geuthrpc) killAndWait() {
	g.geuth.Kill()
	g.geuth.WaitExit()
}

func (g *geuthrpc) callRPC(result interface{}, method string, args ...interface{}) {
	if err := g.rpc.Call(&result, method, args...); err != nil {
		g.geuth.Fatalf("callRPC %v: %v", method, err)
	}
}

func (g *geuthrpc) addPeer(peer *geuthrpc) {
	g.geuth.Logf("%v.addPeer(%v)", g.name, peer.name)
	enode := peer.getNodeInfo().Enode
	peerCh := make(chan *p2p.PeerEvent)
	sub, err := g.rpc.Subscribe(context.Background(), "admin", peerCh, "peerEvents")
	if err != nil {
		g.geuth.Fatalf("subscribe %v: %v", g.name, err)
	}
	...
	select {
	case ev := <-peerCh:
		g.geuth.Logf("%v received event: type=%v, peer=%v", g.name, ev.Type, ev.Peer)
	case err := <-sub.Err():
		g.geuth.Fatalf("%v sub error: %v", g.name, err)
	case <-timeout:
		g.geuth.Error("timeout adding peer after", dur)
	}
}

// Use this function instead of `g.nodeInfo` directly
func (g *geuthrpc) getNodeInfo() *p2p.NodeInfo {
	if g.nodeInfo != nil {
		return g.nodeInfo
	}
...
func (g *geuthrpc) waitSynced() {
	...
	if err != nil {
		g.geuth.Fatalf("%v syncing: %v", g.name, err)
	}
	defer sub.Unsubscribe()
	timeout := time.After(4 * time.Second)
	select {
	case ev := <-ch:
		g.geuth.Log("'syncing' event", ev)
		syncing, ok := ev.(bool)
		if ok && !syncing {
			break
		}
		g.geuth.Log("Other 'syncing' event", ev)
	case err := <-sub.Err():
		g.geuth.Fatalf("%v notification: %v", g.name, err)
		break
	case <-timeout:
		g.geuth.Fatalf("%v timeout syncing", g.name)
		break
	}
}
...
func startGeuthWithIpc(t *testing.T, name string, args ...string) *geuthrpc {
	ipcName := fmt.Sprintf("geuth-%d.ipc", atomic.AddUint32(&nextIPC, 1))
	args = append([]string{"--networkid=42", "--port=0", "--ipcpath", ipcName}, args...)
	t.Logf("Starting %v with rpc: %v", name, args)

	g := &geuthrpc{
		name: name,
		geuth: runGeuth(t, args...),
	}
	// wait before we can attach to it. TODO: probe for it properly
	time.Sleep(1 * time.Second)
	var err error
	ipcpath := ipcEndpoint(ipcName, g.geuth.Datadir)
...
func initGeuth(t *testing.T) string {
	args := []string{"--networkid=42", "init", "./testdata/clique.json"}
	t.Logf("Initializing geuth: %v ", args)
	g := runGeuth(t, args...)
	datadir := g.Datadir
	g.WaitExit()
	return datadir
...
func startLightServer(t *testing.T) *geuthrpc {
	datadir := initGeuth(t)
	t.Logf("Importing keys to geuth")
	runGeuth(t, "--datadir", datadir, "--password", "./testdata/password.txt", "account", "import", "./testdata/key.prv", "--lightkdf").WaitExit()
	account := "0x02f0d131f1f97aef08aec6e3291b957d9efe7105"
	server := startGeuthWithIpc(t, "lightserver", "--allow-insecure-unlock", "--datadir", datadir, "--password", "./testdata/password.txt", "--unlock", account, "--mine", "--light.serve=100", "--light.maxpeers=1", "--nodiscover", "--nat=extip:127.0.0.1", "--verbosity=4")
	return server
}

func startClient(t *testing.T, name string) *geuthrpc {
	datadir := initGeuth(t)
	return startGeuthWithIpc(t, name, "--datadir", datadir, "--nodiscover", "--syncmode=light", "--nat=extip:127.0.0.1", "--verbosity=4")
}
