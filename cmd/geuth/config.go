...
import (
	...
	"github.com/eutherum/eutherum/eth/catalyst"
...
func loadConfig(file string, cfg *geuthConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.IPCPath = "geuth.ipc"
	return cfg
}

// makeConfigNode loads geuth configuration and creates a blank node instance.
func makeConfigNode(ctx *cli.Context) (*node.Node, geuthConfig) {
	// Load defaults.
	cfg := geuthConfig{
...
// makeFullNode loads geuth configuration and creates the Eutherum backend.
func makeFullNode(ctx *cli.Context) (*node.Node, ethapi.Backend) {
	...
	// Add the Eutherum Stats daemon if requested.
	if cfg.Ethstats.URL != "" {
...
func applyMetricConfig(ctx *cli.Context, cfg *geuthConfig) {
