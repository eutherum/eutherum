...
func TestAccountListEmpty(t *testing.T) {
	geuth := runGeuth(t, "account", "list")
	geuth.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	geuth := runGeuth(t, "account", "list", "--datadir", datadir)
	defer geuth.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	geuth := runGeuth(t, "account", "new", "--lightkdf")
	defer geuth.ExpectExit()
	geuth.Expect(`
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	geuth := runGeuth(t, "account", "import", keyfile, "-password", passwordFile)
	defer geuth.ExpectExit()
	geuth.Expect(expected)
}
...
func TestAccountUpdate(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	geuth := runGeuth(t, "account", "update",
...
func TestWalletImport(t *testing.T) {
	geuth := runGeuth(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer geuth.ExpectExit()
	geuth.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(geuth.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	geuth := runMinimalGeuth(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(geuth.StderrText(), m) {
...
// https://github.com/eutherum/eutherum/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	geuth.SetTemplateFunc("keypath", func(file string) string {
