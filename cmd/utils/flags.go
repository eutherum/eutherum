...
// Package utils contains internal helper functions for eutherum commands.
package utils
...
import (
	...
	"github.com/eutherum/eutherum/common/fdlimit"
	...
	"github.com/eutherum/eutherum/eth"
	...
	"github.com/eutherum/eutherum/p2p/netutil"
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.GlobalString(DataDirFlag.Name); path != "" {
		if ctx.GlobalBool(RopstenFlag.Name) {
			// Maintain compatibility with older Geuth configurations storing the
...
// for Geuth and returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `geuth account list`)")
	log.Warn("-------------------------------------------------------------------")
...
func SetP2PConfig(ctx *cli.Context, cfg *p2p.Config) {
	...
	log.Info("Maximum peer count", "UTH", ethPeers, "LES", lightPeers, "total", cfg.MaxPeers)
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	case ctx.GlobalBool(BaikalFlag.Name):
		if !ctx.GlobalIsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 1642 // https://github.com/eutherum/eth1.0-specs/blob/master/network-upgrades/client-integration-testnets/baikal.md
		}
...
// RegisterEthService adds an Eutherum client to the stack.
// The second return value is the full node instance, which may be nil if the
// node is running as a light client.
func RegisterEthService(stack *node.Node, cfg *ethconfig.Config) (ethapi.Backend, *eth.Eutherum) {
	if cfg.SyncMode == downloader.LightSync {
		backend, err := les.New(stack, cfg)
		if err != nil {
			Fatalf("Failed to register the Eutherum service: %v", err)
		}
...
// RegisterEthStatsService configures the Eutherum Stats daemon and adds it to
// the given node.
func RegisterEthStatsService(stack *node.Node, backend ethapi.Backend, url string) {
	if err := ethstats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the Eutherum Stats service: %v", err)
	}
}
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "geuth.", tagsMap)
		}
...
// new format.
//
// e.g. geuth account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// geuth --keystore /tmp/mykeystore --lightkdf account new
//
