...
var commandDeploy = cli.Command{
	Name:  "deploy",
	Usage: "Deploy a new checkpoint euracle contract",
	Flags: []cli.Flag{
		nodeURLFlag,
		clefURLFlag,
		signerFlag,
...
var commandPublish = cli.Command{
	Name:  "publish",
	Usage: "Publish a checkpoint into the euracle",
	Flags: []cli.Flag{
		nodeURLFlag,
		clefURLFlag,
		signerFlag,
...
func deploy(ctx *cli.Context) error {
	...
	fmt.Printf("Deploying new checkpoint euracle:\n\n")
	...
	log.Info("Deployed checkpoint euracle", "address", euracle, "tx", tx.Hash().Hex())
...
func sign(ctx *cli.Context) error {
	...
	if !ctx.GlobalIsSet(nodeURLFlag.Name) {
		...
		_, euracle = newContract(node)
		latest, _, h, err := euracle.Contract().GetLatestCheckpoint(nil)
		if err != nil {
			return err
		}
		if cindex < latest {
	...
	isAdmin := func(addr common.Address) error {
		signers, err := euracle.Contract().GetAllAdmin(nil)
		if err != nil {
			return err
		}
...
// sighash calculates the hash of the data to sign for the checkpoint euracle.
func sighash(index uint64, euracle common.Address, hash common.Hash) []byte {
	buf := make([]byte, 8)
	binary.BigEndian.PutUint64(buf, index)

	data := append([]byte{0x19, 0x00}, append(euracle[:], append(buf, hash[:]...)...)...)
...
func publish(ctx *cli.Context) error {
	// Print the checkpoint euracle's current status to make sure we're interacting
	...
	var (
		...
		addr, euracle = newContract(client)
	...
	// Publish the checkpoint into the euracle
	fmt.Println("Sending publish request to Clef...")
	tx, err := euracle.RegisterCheckpoint(newClefSigner(ctx), checkpoint.SectionIndex, checkpoint.Hash().Bytes(), recent.Number, recent.Hash(), sigs)
	if err != nil {
