...
import (
	...
	"github.com/eutherum/eutherum/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of geuth is "geuth". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Geuth", keep that.
	if name == "geuth" || name == "geuth-testnet" {
		name = "Geuth"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "geuth" instances.
var isOldGeuthResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by geuth 1.4 are used if they exist.
	if warn, isOld := isOldGeuthResource[path]; isOld {
		oldpath := ""
		if c.name() == "geuth" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGeuthResourceWarning, "Using deprecated resource file %s, please move this file to the 'geuth' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "eutherum-keystore")
		ephemeral = keydir
	}
