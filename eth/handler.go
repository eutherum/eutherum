...
import (
	...
	"github.com/eutherum/eutherum/eth/fetcher"
...
// support all the operations needed by the Eutherum chain protocols.
...
// newHandler returns a handler for all Eutherum chain management protocol.
...
func (h *handler) runEthPeer(peer *eth.Peer, handler eth.Handler) error {
	...
	// Execute the Eutherum handshake
	var (
	...
	if err := peer.Handshake(h.networkID, td, hash, genesis.Hash(), forkID, h.forkFilter); err != nil {
		peer.Log().Debug("Eutherum handshake failed", "err", err)
		return err
	}
	...
	peer.Log().Debug("Eutherum peer connected", "name", peer.Name())

	// Register the peer locally
	if err := h.peers.registerPeer(peer, snap); err != nil {
		peer.Log().Error("Eutherum peer registration failed", "err", err)
		return err
	}
...
func (h *handler) removePeer(id string) {
	...
	if peer == nil {
		logger.Error("Eutherum peer removal failed", "err", errPeerNotRegistered)
		return
	}
	// Remove the `eth` peer if it exists
	logger.Debug("Removing Eutherum peer", "snap", peer.snapExt != nil)
	...
	if err := h.peers.unregisterPeer(id); err != nil {
		logger.Error("Eutherum peer removal failed", "err", err)
	}
...
func (h *handler) Stop() {
	...
	log.Info("Eutherum protocol stopped")
}
