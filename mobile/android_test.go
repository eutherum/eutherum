...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Geuth.LightScryptN, Geuth.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.eutherum", new NodeConfig());
			node.start();
			...
			EutherumClient ec = node.getEutherumClient();
	...
	// signatures too. Regression test for eutherum issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "geuth-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/eutherum/geuthtest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Geuth and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.eutherum", "github.com/eutherum/eutherum/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "geuth.aar"), "geuth.aar")

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "eutherum", "geuthtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.eutherum.geuthtest"
