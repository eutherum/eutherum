...
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data interface{}) (*SignDataRequest, bool, error) {
	var (
		req          *SignDataRequest
		useEutherumV = true // Default to use V = 27 or 28, the legacy Eutherum format
	)
	mediaType, _, err := mime.ParseMediaType(contentType)
	if err != nil {
		return nil, useEutherumV, err
	}
	...
	case ApplicationClique.Mime:
		// Clique is the Eutherum PoA standard
		stringData, ok := data.(string)
		if !ok {
			return nil, useEutherumV, fmt.Errorf("input for %v must be an hex-encoded string", ApplicationClique.Mime)
		}
	...
	default: // also case TextPlain.Mime:
		// Calculates an Eutherum ECDSA signature for:
		// hash = keccak256("\x19${byteVersion}Eutherum Signed Message:\n${message length}${message}")
		// We expect it to be a string
		if stringData, ok := data.(string); !ok {
			return nil, useEutherumV, fmt.Errorf("input for text/plain must be an hex-encoded string")
		} else {
