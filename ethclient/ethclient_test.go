...
// Verify that Client implements the eutherum interfaces.
var (
	_ = eutherum.ChainReader(&Client{})
	_ = eutherum.TransactionReader(&Client{})
	_ = eutherum.ChainStateReader(&Client{})
	_ = eutherum.ChainSyncReader(&Client{})
	_ = eutherum.ContractCaller(&Client{})
	_ = eutherum.GasEstimator(&Client{})
	_ = eutherum.GasPricer(&Client{})
	_ = eutherum.LogFilterer(&Client{})
	_ = eutherum.PendingStateReader(&Client{})
	// _ = eutherum.PendingStateEventer(&Client{})
	_ = eutherum.PendingContractCaller(&Client{})
)
...
func TestToFilterArg(t *testing.T) {
	...
	for _, testCase := range []struct {
		name   string
		input  eutherum.FilterQuery
		output interface{}
		err    error
	}{
		{
			"without BlockHash",
			eutherum.FilterQuery{
				Addresses: addresses,
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	// Create Eutherum Service
	config := &ethconfig.Config{Genesis: genesis}
	config.Ethash.PowMode = ethash.ModeFake
	ethservice, err := eth.New(n, config)
	if err != nil {
		t.Fatalf("can't create new eutherum service: %v", err)
	}
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == eutherum.NotFound {
		t.Fatal("error should not be nil/notfound")
	}
	// Test tx in block not found
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 1); err != eutherum.NotFound {
		t.Fatal("error should be eutherum.NotFound")
	}
}
