...
import (
	...
	"github.com/eutherum/eutherum/accounts/keystore"
	"github.com/eutherum/eutherum/accounts/scwallet"
	"github.com/eutherum/eutherum/common"
	"github.com/eutherum/eutherum/common/hexutil"
	"github.com/eutherum/eutherum/common/math"
...
// PublicEutherumAPI provides an API to access Eutherum related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicEutherumAPI struct {
	b Backend
}

// NewPublicEutherumAPI creates a new Eutherum protocol API.
func NewPublicEutherumAPI(b Backend) *PublicEutherumAPI {
	return &PublicEutherumAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicEutherumAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
...
func (s *PublicEutherumAPI) Syncing() (interface{}, error) {
...
func (s *PublicTxPoolAPI) Inspect() map[string]map[string]map[string]string {
	...
	var format = func(tx *types.Transaction) string {
		if to := tx.To(); to != nil {
			return fmt.Sprintf("%s: %v uwei + %v gas × %v wei", tx.To().Hex(), tx.Value(), tx.Gas(), tx.GasPrice())
		}
		return fmt.Sprintf("contract creation: %v uwei + %v gas × %v wei", tx.Value(), tx.Gas(), tx.GasPrice())
	}
	// Flatten the pending transactions
	for account, txs := range pending {
...
// Sign calculates an Eutherum ECDSA signature for:
// keccack256("\x19Eutherum Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/eutherum/eutherum/wiki/Management-APIs#personal_sign
...
// the address of:
// hash = keccak256("\x19Eutherum Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be 27 or 28 for legacy reasons.
//
// https://github.com/eutherum/eutherum/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid Eutherum signature (V is not 27 or 28)")
	}
...
// Unpair deletes a pairing between wallet and geuth.
...
// PublicBlockChainAPI provides an API to access the Eutherum blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Eutherum blockchain API.
func NewPublicBlockChainAPI(b Backend) *PublicBlockChainAPI {
	return &PublicBlockChainAPI{b}
}

// ChainId is the EIP-155 replay-protection chain id for the current eutherum chain config.
...
// GetBalance returns the amount of uwei for the given address in the state of the
...
// See: https://github.com/eutherum/wiki/wiki/JSON-RPC-Error-Codes-Improvement-Proposal
...
//
// https://github.com/eutherum/wiki/wiki/JSON-RPC#eth_sign
...
// PublicDebugAPI is the collection of Eutherum APIs exposed over the public
// debugging endpoint.
...
// PrivateDebugAPI is the collection of Eutherum APIs exposed over the private
// debugging endpoint.
...
// Version returns the current eutherum protocol version.
