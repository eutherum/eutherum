...
// Package accounts implements high level Eutherum account management.
package accounts
...
// Account represents an Eutherum account located at a specific location defined
// by the optional URL field.
type Account struct {
	Address common.Address `json:"address"` // Eutherum account address derived from the key
...
type Wallet interface {
	...
	// chain state reader.
	SelfDerive(bases []DerivationPath, chain eutherum.ChainStateReader)
...
//   keccak256("\x19Eutherum Signed Message:\n"${message length}${message}).
//
...
func TextAndHash(data []byte) ([]byte, string) {
	msg := fmt.Sprintf("\x19Eutherum Signed Message:\n%d%s", len(data), string(data))
